...
public class WorksController(IMediator mediator, IMapper mapper) : WorkshopBaseController(mediator, mapper)
{
    ...
    [HttpPost("{id}/work")]
    public async Task<WorkInOrderResult> AddWork(
        [FromRoute] Guid id,
        [FromBody] CreateWorkInOrderCommand command
    )
    {
        command.Actor = await GetUser();
        command.OrderId = id;
        return _mapper.Map<WorkInOrderResult>(await _mediator.Send(command));
    }

    [HttpPatch("{id}/work/{workId}")]
    public async Task<WorkInOrderResult> UpdateWork(
        [FromRoute] Guid id,
        [FromRoute] Guid workId,
        [FromBody] UpdateWorkInOrderCommand command
    )
    {
        command.Actor = await GetUser();
        command.OrderId = id;
    ...
    {
        var command = new DeleteWorkInOrderCommand { Actor = await GetUser(), OrderId = id, WorkId = workId };
        return new MessageResult(await _mediator.Send(command));
    }
}
